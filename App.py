...
class App:
    def __init__(self, root):
        ...
        self.generate_button = Button(canvas, text="Generate", command=lambda: set_vertex_edge(self))
        self.generate_button.place(x=WIDTH / 5, y=HEIGHT / 2)

        self.v_entry = Entry(canvas, width=5)
        self.v_entry.place(x=WIDTH / 5, y=HEIGHT / 3)
        self.e_entry = Entry(canvas, width=5)
        self.e_entry.place(x=WIDTH / 5, y=HEIGHT / 3 + 20)
        ...
        def draw_label(self, name, x, y, bg, w, h):
            ...
            self.label = Label(canvas, textvariable=self.label_text, bg=bg)
            self.label.place(x=x - 6, y=y - 9)
            self.label_text.set(name)

        # draws vertex centered around x, y coordinates
        def draw_vertex(self, name, x, y, r):
            # draw vertices
        ...
        def set_vertex_edge(self):
            v.set(self.v_entry.get())
            e.set(self.e_entry.get())
            canvas.delete("all")
            generate_graph(self, v.get(), e.get())
